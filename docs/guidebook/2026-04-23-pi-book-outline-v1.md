...
1. 00 先排除“又一个 coding agent”的误读
2. 01 定总论点：`pi` 是可编程宿主
3. 02 给宿主底座：session tree 是 runtime state
4. 03 展开委派型外化：subagent 是独立 runtime 委派
5. 04 展开模态切换型外化：plan mode 是主 runtime 的 mode switch
6. 05 收成宿主平台：extension surface 覆盖 tool / UI / provider / compaction
7. 06 回到产品路线：它适合谁、不适合谁，以及和强成品 agent 的关系
