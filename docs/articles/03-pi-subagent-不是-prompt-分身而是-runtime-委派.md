...
这种做法有用，但不是 `pi` 这个 subagent 示例里最值得看的部分。
...
到了 subagent，这个判断又多了一层证据：`pi` 不只保存 runtime，它还允许高级工作流从 extension 层接进来。
...
这和 Claude Code / cc 的感觉不一样。cc 里的 subagent/fork 更像 core runtime 的内部机制：主 agent 内部分叉、共享 prefix、复用 prompt cache、控制 fork child rules。这是一条很强的官方产品路线。
...
agent definition 不是一个展示标签。它会把自己的 system prompt 写入临时文件，再附加到子进程里。

第三个是 `--tools` 和 `--model`。

也就是说，不同 agent 不只是“性格不同”。它们还可以有不同的工具集合和模型配置。

所以 `pi` 的 subagent 至少有这几层隔离：
...
这段代码的含义很直接：父进程拿到的是另一个 agent runtime 的结构化事件流，不只是一句最终回复。
...
它具备一个多 agent orchestration 的最小骨架：
...
`pi` 这条路线值得看，原因不在某个内建功能有多完整，而在这些工作流不必都进 core 才能运行。

---

## 5. project agents 暴露了宿主路线的信任边界

还有一个容易被忽略的点：agent 从哪里来。
...
只要一个 repo 能放 `.pi/agents/*.md`，它就能提供 repo-local prompt。这个 prompt 可能让 agent 读文件、跑命令、修改代码。也就是说，project agent 天然落在信任边界上。
...
所以两边的差异不在于“谁有没有 subagent”，而在于实现路径不同。
...
第 03 章看 subagent，又能看到另一层：`pi` 也没有把所有高级工作流都写死在 core product 里。
...
源码会告诉你更具体的事：它把 subagent 放在 extension 层实现。它想做的不是不断往 core 里堆功能；更重要的是，让 agent 工作流有地方继续生长。
