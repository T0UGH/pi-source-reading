...
一个讲委派，一个讲切模式，看起来不是一回事。放到 extension 这一层看，它们指向同一个设计取向：

> `pi` 把一部分高级工作流从 core product 里拿出来，放到 extension runtime surface 上。

这一章把问题再往上提一层：如果 tool、UI、provider、compaction、session lifecycle 都可以外化，agent 产品会变成什么样？
...
## 1. extension 不止是插件点，更是 runtime interface

很多产品说自己支持插件，实际可能只是允许用户多加几个 tool。

`pi` 的 extension surface 大得多。
...
这已经超过“给 agent 加几个小功能”的插件系统，更像一组 runtime interface。
...
`pi` 的 `registerProvider()` 明确把这些交给了 extension。
...
这个例子里，provider 已经超出配置项，成了 extension 可以接管的一层基础设施。

这对系统型用户很有用。真实环境里的模型访问，常常不是“填一个官方 API key”就结束。你可能还需要：
...
`pi` 把 `session_before_compact` 暴露了出来。
...
extension 收到的不是一个只读通知。它可以取消，也可以直接返回自定义的 compaction 结果。
...
到这一步，compaction 也变成了 host behavior。

这比“支持自定义摘要 prompt”又往里走了一步。

extension 可以参与上下文治理，而上下文治理会直接影响长期 agent 工作的质量。

如果说 session tree 是 runtime state 的存储底座，那么 compaction hook 就是 extension 改写 runtime state 的入口之一。
...
第 02 章讲的 session tree，到这里才继续展开。

当 session 能保存 extension state，agent 产品就不再只是一个对话窗口。它更像一个可以恢复、分叉、压缩、扩展的工作台。
...
这也是前面一直在强调的：`pi` 的卖点不是“比 cc 多了什么功能”。
...
这类用户不太关心“官方菜单里有没有这个功能”。他们更在意：

> 我能不能把自己的系统接进去？我能不能把自己的 workflow 长出来？

对这类人来说，`pi` 的 host/runtime 路线有吸引力。
...
第 03 章的 subagent 已经碰到这个问题：project agents 默认不加载，启用 project/both 时还要确认。

第 04 章 plan mode 也碰到这个问题：只读不能靠 prompt 自觉，需要 runtime enforcement 做 bash allowlist。

`pi` 的宿主路线有一个天然代价：

> 它把更多能力交给用户和 extension，也把更多治理责任放到了宿主边界上。

这很难简单归成缺点或优点，它就是这条产品路线的价格。
