...
这篇不打算把 `pi` 讲成“又一个 terminal coding agent”。

要回答的只有一个核心问题：

> **为什么 `pi` 最值得研究的地方，不是它又做了一套 agent 功能，而是它把 coding agent 做成了一个可编程宿主？**

如果把这个问题讲清楚，后面再看它的 session、subagent、plan mode、provider，才不会读歪。
...
这也是为什么它对会自己搭系统的人特别值得研究。

---

## 先看一张路线对照图

这篇最重要的判断，其实可以先压成一张图来看。
...
> `pi` 不是在拼命往 core 里塞更多高级工作流，而是在把 **高级工作流应该长在哪里** 这件事重新定义。

它想做的不是：
- 一个封闭成品

而是：
- 一个最小但足够强的 coding-agent host

---

## 第一层证据：README 自己就把产品哲学说白了

`packages/coding-agent/README.md` 里有两句特别关键：
...
- 默认能力已经够强
- 但高级工作流不应该优先固化成产品内核
- 而是让用户通过 extensions / packages 自己长出来
...
> **官方先定义一套比较强的 agent runtime 和 worker 体系，再把高级工作流做进核心产品。**
...
> extension 在 `pi` 里不是“插件附属物”，而是宿主的正式能力表面。
...
如果一个系统把这些层都开放给 extension，意味着什么？

意味着它不是在说：
- “你可以给我的产品打一点补丁”

而是在说：
- “你可以把自己的 runtime 行为长进我的宿主里”
...
这说明它不是 prompt cosplay，而是：

> **宿主之上的受控委派 runtime**

关键不在于“它也能 subagent”，而在于：
...
也就是说，plan mode 在 `pi` 这里不是一个 agent 角色，而是一种：

> **宿主可切换的工作模式**

这和 cc 非常不一样。

在 cc 里，plan 更像官方内建 worker 体系的一部分；在 `pi` 里，plan 是 extension 驱动的 runtime mode。
...
都不是附属功能，而是建立在同一个 runtime 底座上。

这也进一步说明：

> `pi` 不只是给你一个聊天壳，而是在给你一个长期 agent runtime。`

而一旦 session 是 runtime 底座，extension 才真的有地方长状态机、长工作模式、长委派器。
...
- 我能不能把自己的规则层接进去
- 我能不能把自己的执行边界接进去
- 我能不能不改上游核心源码，还长出自己的 plan/subagent/provider/UI
...
这就是为什么它对贵平这种会自己搭系统、在意知识层/规则层/执行层边界的人，研究价值很高。

---

## 这篇的最终判断

如果把整篇文章最后收成一句话，我会用这句：

> **Claude Code 更像一个已经做得很强的官方 agent 成品，而 `pi` 更像一个让你自己长出 agent 工作流的宿主。**

而这正是 `pi` 最值得读、也最容易被普通“功能视角”忽略的地方。
